...
function attemptsText(snapshot) {
  if (snapshot.isAdmin) return '**∞ · GM**';
  return `**${snapshot.attemptsRemaining} / ${DUNGEON_DAILY_ATTEMPTS}**`;
}

function petLine(snapshot) {
  if (!snapshot.pet) return `${PET} **Linh Thú:** Chưa xuất chiến · **+0% Vượt Ải**`;
...
function formationLine(snapshot) {
  ...
  if (!info?.formation) return `${FORMATION} **Trận Pháp:** Chưa bố trí · **Giảm 0% Sinh Mệnh tổn thất**`;
...
export function buildDungeonMainEmbed(user, snapshot, notice = null) {
  ...
  const lines = [
    ...
    active ? `Đang khiêu chiến: **Tầng ${active.floor}** · ${HP} **${active.hp}% Sinh Mệnh**` : `Tầng kế tiếp: **Tầng ${snapshot.floor}**`, '',
    petLine(snapshot), formationLine(snapshot), '',
    `${DANGER} **Quy Tắc**`,
    ...
    `• ${HP} Sang **ngày mới**, Sinh Mệnh Bí Cảnh được **hồi đầy 100%**.`,
  ...
  return style(new EmbedBuilder().setTitle(title('BÍ CẢNH · 秘境')).setDescription(lines.join('\n')));
}
...
export function buildDungeonMainRows(ownerId, snapshot) {
  ...
  const enter = button(ownerId, active ? 'floor' : 'start', active ? 'Tiếp Tục Bí Cảnh' : 'Tiến Vào Bí Cảnh', active ? BUTTON_EMOJI.gate : BUTTON_EMOJI.dungeon).setDisabled(cannotStart);
...
export function buildDungeonFloorEmbed(user, snapshot) {
  ...
  return style(new EmbedBuilder()
    .setTitle(title(guardian ? `THỦ HỘ GIẢ · TẦNG ${floor}` : `BÍ CẢNH · TẦNG ${floor}`))
    .setDescription([
      `${guardian ? MONSTER : GATE} **${majorBoss ? 'Cổ Cảnh Đại Thủ Hộ' : guardian ? 'Thủ Hộ Giả Bí Cảnh' : 'Cổ Cảnh Vô Danh'}**`, '',
      `${HP} **Sinh Mệnh Đạo Hữu**`, `\`${hpBar(run?.hp ?? 100)}\` **${run?.hp ?? 100}%**`, '',
      `${COMBAT} **Tỷ Lệ Vượt Ải Cơ Bản:** **${snapshot.baseChance}%**`, petLine(snapshot),
      `${VICTORY} **Tỷ Lệ Vượt Ải Cuối:** **${snapshot.successChance}%**${snapshot.successChance >= 100 ? ' · Chắc chắn thành công' : ''}`, '',
      formationLine(snapshot), '', `${CHEST} **Phần thưởng khi vượt tầng**`,
      `• ${STONE} Linh Thạch`, `• ${HERB} Thiên Linh Thảo`, `• ${ORE} Huyền Thiết`,
      `• ${MANUAL} Vô Danh Kiếm Phổ`, `• ${TALISMAN} Thượng Cổ Phù`,
      `• ${FORMATION_ESSENCE} Trận Văn`, `• ${ESSENCE} Bí Cảnh Tinh Hoa`, '',
...
export function buildDungeonSuccessEmbed(user, result) {
  const lines = [
    ...
    result.pet ? `${result.pet.emoji || PET} ${result.pet.name}: **+${result.petBonus}% Vượt Ải**` : `${PET} Linh Thú: **+0%**`,
    ...
    `${CHEST} **Chiến Lợi Phẩm**`, ...rewardLines(result.reward), '', SEPARATOR, '',
    result.exhausted ? `${DEFEAT} **Sinh Mệnh đã cạn sau khi phá tầng. Lượt Bí Cảnh kết thúc.**` : `${GATE} Tầng tiếp theo: **Tầng ${result.nextFloor}**`, '',
    '*Bí Cảnh chỉ ban tài nguyên, không ban Tu Vi.*',
  ];
  return style(new EmbedBuilder().setTitle(title('PHÁ TẦNG THÀNH CÔNG')).setDescription(lines.join('\n')));
}

export function buildDungeonSuccessRows(ownerId, result) {
  if (result.exhausted) return [new ActionRowBuilder().addComponents(
...
export function buildDungeonFailureEmbed(result) {
  return style(new EmbedBuilder().setTitle(title('BÍ CẢNH · THẤT THỦ')).setDescription([
    `${DEFEAT} **Đạo tâm thất thủ tại Tầng ${result.floor}.**`, '',
    `${COMBAT} Tỷ lệ vượt ải: **${result.successChance}%**`,
    result.pet ? `${result.pet.emoji || PET} ${result.pet.name}: **+${result.petBonus}% Vượt Ải**` : `${PET} Linh Thú: **+0%**`,
    `${FORMATION} ${result.formation.formation?.name || 'Chưa bố trí'} · Lv.${result.formation.level || 0}: **Giảm ${result.formation.reduction || 0}% tổn thất**`, '',
    `${HP} Sinh Mệnh trước giao chiến: **${result.hpBefore}%**`, `${HP} Sinh Mệnh sau thất thủ: **${result.hpAfter}%**`,
    `Tổn thất: **-${result.hpLoss}%**`, '',
    `${DANGER} Tầng ${result.floor} **chưa được tính là vượt qua** và không nhận phần thưởng tầng này.`,
    'Những tài nguyên đã nhận từ các tầng trước vẫn được giữ nguyên.', '', '*Một lần thất bại chưa đủ đoạn tiên đồ.*',
  ].join('\n')));
}
...
export function buildDungeonShopEmbed(snapshot, notice = null) {
  ...
  return style(new EmbedBuilder().setTitle(title('KHO BÍ CẢNH')).setDescription(lines.join('\n')));
}

export function buildDungeonShopRows(ownerId, snapshot) {
  return [
    new ActionRowBuilder().addComponents(
      button(ownerId, 'buy_thien_linh_thao', 'Thiên Linh Thảo', BUTTON_EMOJI.herb).setDisabled(snapshot.state.essence < DUNGEON_SHOP.thien_linh_thao.price),
      button(ownerId, 'buy_huyen_thiet', 'Huyền Thiết', BUTTON_EMOJI.ore).setDisabled(snapshot.state.essence < DUNGEON_SHOP.huyen_thiet.price),
      button(ownerId, 'buy_vo_danh_kiem_pho', 'Vô Danh Kiếm Phổ', BUTTON_EMOJI.manual).setDisabled(snapshot.state.essence < DUNGEON_SHOP.vo_danh_kiem_pho.price),
      button(ownerId, 'buy_co_phu', 'Thượng Cổ Phù', BUTTON_EMOJI.talisman).setDisabled(snapshot.state.essence < DUNGEON_SHOP.co_phu.price),
      button(ownerId, 'buy_tran_van', 'Trận Văn', BUTTON_EMOJI.formationEssence).setDisabled(snapshot.state.essence < DUNGEON_SHOP.tran_van.price),
    ),
    new ActionRowBuilder().addComponents(button(ownerId, 'main', 'Quay Lại Bí Cảnh', BUTTON_EMOJI.dungeon)),
  ];
}
...
export function buildDungeonLeaderboardEmbed(entries) {
  ...
  return style(new EmbedBuilder().setTitle(title('BÍ CẢNH · TIÊN BẢNG')).setDescription([
    `${DUNGEON} **Xếp hạng theo tầng cao nhất đã vượt.**`, '', ...lines,
  ].join('\n')));
}

export function buildDungeonLeaderboardRows(ownerId) {
  return [new ActionRowBuilder().addComponents(button(ownerId, 'main', 'Quay Lại Bí Cảnh', BUTTON_EMOJI.dungeon))];
}
