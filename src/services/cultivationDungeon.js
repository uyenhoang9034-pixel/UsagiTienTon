...
export const DUNGEON_SHOP = {
  ...
  tran_van: {
    ...
    name: 'Trận Văn',
    price: 90,
    type: 'formation_essence',
  },
};

function stateKey(guildId, userId) {
  return `${DUNGEON_KEY_PREFIX}${guildId}:${userId}`;
}

function guildPrefix(guildId) {
  return `${DUNGEON_KEY_PREFIX}${guildId}:`;
}
...
function normalizeState(raw, guildId, userId) {
  ...
  const activeRun = source.activeRun && typeof source.activeRun === 'object'
    ? {
        floor: Math.max(1, Math.floor(Number(source.activeRun.floor) || 1)),
        hp: clamp(Math.round(Number(source.activeRun.hp) || 0), 0, DUNGEON_MAX_HP),
        startedAt: Number(source.activeRun.startedAt) || Date.now(),
      }
    : null;
  ...
  return {
    ...
    userId,
    dayKey: day,
    attemptsUsed: sameDay
      ? clamp(Math.floor(Number(source.attemptsUsed) || 0), 0, DUNGEON_DAILY_ATTEMPTS)
      : 0,
...
export async function getDungeonState(client, guildId, userId) {
  ...
  if (!raw || raw.dayKey !== state.dayKey || Number(raw.version) !== 1) {
...
function getPetSuccessBonus(profile) {
  ...
  return {
    pet,
    bonus: DUNGEON_PET_SUCCESS_BONUS[pet.rarity] || 0,
  };
}
...
async function getFormationProtection(client, guildId, userId) {
  ...
  return {
    ...
    gradeBonus,
    levelBonus,
    reduction: gradeBonus + levelBonus,
  };
}
...
function getPlayerPowerStep(profile) {
  return Math.max(
    0,
    Math.floor(Number(profile?.realmIndex) || 0) * 4 +
      Math.floor(Number(profile?.stageIndex) || 0),
  );
}
...
function getBaseSuccessChance(profile, floor) {
  ...
  const recommendedStep = getFloorRecommendedStep(floor);
  ...
  return clamp(Math.round(chance), 25, 95);
}

function getRawHpLoss(floor, success) {
  const safeFloor = Math.max(1, Math.floor(Number(floor) || 1));
  const depthBonus = Math.min(12, Math.floor((safeFloor - 1) / 10));
  const guardianBonus = safeFloor % 5 === 0 ? 8 : 0;
  const normalLoss = 8 + depthBonus + guardianBonus + randomInt(0, 4);
...
function applyFormationProtection(rawLoss, reductionPercent) {
  const reduced = Math.round(
    Math.max(0, rawLoss) * (1 - clamp(reductionPercent, 0, 100) / 100),
  );
...
function getEssenceRange(floor) {
  ...
  if (band <= ranges.length) {
    return ranges[band - 1];
  }
...
async function applyFloorReward(client, guildId, userId, profile, reward) {
  ...
  formationState.formationEssence =
    Math.max(0, Number(formationState.formationEssence) || 0) + reward.formationEssence;
...
export async function getDungeonSnapshot(client, guildId, userId, { isAdmin = false } = {}) {
  const [state, profile, formation] = await Promise.all([
    getDungeonState(client, guildId, userId),
    getCultivationProfile(client, guildId, userId),
    getFormationProtection(client, guildId, userId),
  ]);
  ...
  return {
    ...
    successChance,
    isAdmin,
    attemptsRemaining: isAdmin
      ? Infinity
      : Math.max(0, DUNGEON_DAILY_ATTEMPTS - state.attemptsUsed),
  };
}
  ...
  return Mutex.runExclusive(`cultivation:dungeon:start:${guildId}:${userId}`, async () => {
    ...
    if (state.activeRun) {
      return {
        ok: true,
        resumed: true,
        ...(await getDungeonSnapshot(client, guildId, userId, { isAdmin })),
      };
    }

    if (!isAdmin && state.attemptsUsed >= DUNGEON_DAILY_ATTEMPTS) {
      return {
        ok: false,
        reason: 'daily_limit',
        ...(await getDungeonSnapshot(client, guildId, userId, { isAdmin })),
...
export async function challengeDungeonFloor(client, guildId, userId, { isAdmin = false } = {}) {
  return Mutex.runExclusive(`cultivation:dungeon:fight:${guildId}:${userId}`, async () => {
    ...
    const [profile, formation] = await Promise.all([
      getCultivationProfile(client, guildId, userId),
      getFormationProtection(client, guildId, userId),
    ]);
    ...
    // Lưu tiến độ trước khi cấp thưởng để cùng một tầng không thể nhận lại do double-click.
    await saveDungeonState(client, state);
...
export async function buyDungeonShopItem(
  client,
  guildId,
  userId,
  itemId,
...
export async function getDungeonLeaderboard(client, guildId, limit = 10) {
  ...
  const keys = typeof client?.db?.list === 'function'
    ? await client.db.list(prefix)
    : [];
