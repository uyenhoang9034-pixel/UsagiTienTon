...
function makeTargets(g,u,p,date){const roll=rng(g+':'+u+':'+date+':bounty-v1'),used=new Set();return[0,1,2].map((_,slot)=>{let stars=weighted(STAR_WEIGHTS,roll).v;if(slot===2&&stars<2)stars=2;const monster=weighted(MONSTERS.filter(m=>!used.has(m.id)),roll,'weight');used.add(monster.id);const step=Math.max(0,progression(p)+[-2,-1,0,1,2][stars-1]);return{slot,id:date+':'+slot+':'+monster.id,monsterId:monster.id,name:monster.name,stars,rare:stars>=4,step,realm:targetDisplay(step),staminaCost:7+stars*3,reward:rewardFor(step,stars),completed:false};});}
function normalize(raw,date,targets){if(!raw||raw.date!==date)return{version:1,date,points:Math.max(0,Number(raw?.points)||0),completed:{},targets};const completed=raw.completed&&typeof raw.completed==='object'?raw.completed:{};return{...raw,version:1,date,points:Math.max(0,Number(raw.points)||0),completed,targets:targets.map(t=>({...t,completed:completed[t.id]===true}))};}
...
async function combatInfo(client,g,u,p,t){const cave=await getSafeCavePetBonus(client,g,u),pet=amplifySafePetEffect(getPetEffectValue(p,'combat_success_bonus'),cave,{cap:.9}),formation=await getFormationGameplayBonus(client,g,u),form=Math.max(0,Number(formation.effects?.adventureBonus)||0),diff=t.step-progression(p),base=Math.max(.22,Math.min(.9,.72-diff*.105)),chance=Math.max(.1,Math.min(.95,base+pet+form*.35));return{chance,danger:danger(chance),petBonus:pet,formationBonus:form,activePet:getActivePet(p)};}
export async function getBountyBoard(client,g,u){const profile=await getCultivationProfile(client,g,u),s=await getState(client,g,u,profile),targets=[];for(const t of s.targets)targets.push({...t,...await combatInfo(client,g,u,profile,t)});return{ok:true,profile,points:s.points,targets,resetRemaining:getBountyResetRemaining()};}
export async function getBountyTarget(client,g,u,slot){const board=await getBountyBoard(client,g,u),target=board.targets[Number(slot)];return target?{...board,target}:{ok:false,reason:'invalid_target',...board};}
export async function fightBounty(client,g,u,slot){return Mutex.runExclusive('cultivation:'+g+':'+u,async()=>{const profile=await getCultivationProfile(client,g,u),s=await getState(client,g,u,profile),target=s.targets[Number(slot)];if(!target)return{ok:false,reason:'invalid_target',profile};if(s.completed[target.id])return{ok:false,reason:'completed',profile,target:{...target,completed:true},points:s.points};if((Number(profile.stamina)||0)<target.staminaCost)return{ok:false,reason:'stamina',profile,target,points:s.points};const info=await combatInfo(client,g,u,profile,target);profile.stamina=Math.max(0,(Number(profile.stamina)||0)-target.staminaCost);const success=Math.random()<info.chance;let drop=null,reward={cultivation:0,stones:0,points:0};if(success){reward={...target.reward};const cave=await getSafeCavePetBonus(client,g,u),rewardPet=amplifySafePetEffect(getPetEffectValue(profile,'combat_reward_bonus'),cave);reward.cultivation=Math.round(reward.cultivation*(1+rewardPet));reward.stones=Math.round(reward.stones*(1+rewardPet));profile.cultivation=Math.max(0,Number(profile.cultivation)||0)+reward.cultivation;profile.totalCultivation=Math.max(0,Number(profile.totalCultivation)||0)+reward.cultivation;profile.spiritStones=Math.max(0,Number(profile.spiritStones)||0)+reward.stones;s.points+=reward.points;s.completed[target.id]=true;const dc=[.04,.08,.14,.24,.38][target.stars-1];if(Math.random()<dc){const itemId=target.stars>=4?(Math.random()<.35?'vo_danh_kiem_pho':'huyen_thiet'):(Math.random()<.55?'huyen_thiet':'thien_linh_thao');if(CULTIVATION_ITEMS[itemId]&&addInventoryItem(profile,itemId,1))drop={itemId,item:CULTIVATION_ITEMS[itemId],quantity:1};}}const saved=await saveCultivationProfile(client,profile);await client.db.set(key(g,u),s);return{ok:true,success,profile:saved,target:{...target,completed:success},points:s.points,reward,drop,...info};});}
