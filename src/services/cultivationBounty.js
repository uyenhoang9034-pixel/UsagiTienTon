...
export async function getBountyBoard(client,g,u){const profile=await getCultivationProfile(client,g,u),s=await getState(client,g,u,profile),formation=await getFormationGameplayBonus(client,g,u),targets=[];for(const t of s.targets){const staminaResult=applyFormationStaminaReduction(t.staminaCost,formation.effects);targets.push({...t,actualStaminaCost:staminaResult.total,staminaSaved:staminaResult.saved,...await combatInfo(client,g,u,profile,t)});}return{ok:true,profile,points:s.points,targets,resetRemaining:getBountyResetRemaining()};}
