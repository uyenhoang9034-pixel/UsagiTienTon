...
export async function resetBountyToday(client,g,u){const profile=await getCultivationProfile(client,g,u),date=dayKey(),raw=await client.db.get(key(g,u),null),points=Math.max(0,Number(raw?.points)||0),targets=makeTargets(g,u,profile,date),state={version:1,date,points,completed:{},targets};await client.db.set(key(g,u),state);return getBountyBoard(client,g,u);}
