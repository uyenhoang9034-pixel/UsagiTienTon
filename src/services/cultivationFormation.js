...
export const FORMATION_DEFINITIONS = {
  five_elements: { id: 'five_elements', name: 'Tiểu Ngũ Hành Trận', rarity: 'Phàm', unlockInsight: 0, slots: 5, pattern: ['metal', 'water', 'wood', 'fire', 'earth'], effect: 'Tăng hiệu quả Tu Luyện khi Ngũ Hành vận chuyển hoàn chỉnh.' },
  wind_lightning: { id: 'wind_lightning', name: 'Phong Lôi Dẫn Thiên Trận', rarity: 'Hoàng', unlockInsight: 100, slots: 4, pattern: ['wind', 'lightning', 'wind', 'lightning'], effect: 'Tăng khả năng ứng biến trong Thám Hiểm và Bí Cảnh.' },
  frozen_spirit: { id: 'frozen_spirit', name: 'Huyền Băng Tỏa Linh Trận', rarity: 'Huyền', unlockInsight: 250, slots: 4, pattern: ['water', 'ice', 'ice', 'water'], effect: 'Ổn định linh lực, thiên về tiết kiệm Thể Lực.' },
  yin_yang: { id: 'yin_yang', name: 'Âm Dương Lưỡng Nghi Trận', rarity: 'Địa', unlockInsight: 500, slots: 4, pattern: ['yin_yang', 'spirit', 'spirit', 'yin_yang'], effect: 'Điều hòa Âm Dương, hỗ trợ Đột Phá và lĩnh ngộ.' },
  chaos_unity: { id: 'chaos_unity', name: 'Hỗn Độn Quy Nhất Trận', rarity: 'Thiên', unlockInsight: 1000, slots: 5, pattern: ['metal', 'wood', 'chaos', 'water', 'fire'], effect: 'Hỗn Độn làm mắt trận, khuếch đại các cộng hưởng đang vận hành.' },
};
...
function stateKey(guildId, userId) { return `${FORMATION_KEY_PREFIX}${guildId}:${userId}`; }

function normalizeState(data) {
  const state = { ...DEFAULT_STATE, ...(data && typeof data === 'object' ? data : {}) };
  state.unlockedFormationIds = Array.from(new Set(['five_elements', ...(Array.isArray(state.unlockedFormationIds) ? state.unlockedFormationIds : [])]));
  state.formationLevels = { ...DEFAULT_STATE.formationLevels, ...(state.formationLevels || {}) };
  state.layouts = { ...DEFAULT_STATE.layouts, ...(state.layouts || {}) };
  if (!FORMATION_DEFINITIONS[state.activeFormationId]) state.activeFormationId = 'five_elements';
  return state;
}
...
export async function getFormationState(client, guildId, userId) {
  ...
  if (!stored) await setDatabaseValue(client, stateKey(guildId, userId), state);
  return state;
}
...
export function getActiveFormation(state) { return FORMATION_DEFINITIONS[state?.activeFormationId] || FORMATION_DEFINITIONS.five_elements; }
export function getFormationLevel(state, formationId = state?.activeFormationId) { return Math.max(1, Number(state?.formationLevels?.[formationId]) || 1); }
...
export function getFormationResonance(state) {
  ...
  if (fiveBasic.every((id) => unique.has(id))) { lines.push('Ngũ Hành Tuần Hoàn'); multiplier += 0.12; }
  if (unique.has('wind') && unique.has('lightning')) { lines.push('Phong Lôi Đồng Hành'); multiplier += 0.08; }
  if (unique.has('water') && unique.has('ice')) { lines.push('Hàn Triều Tỏa Linh'); multiplier += 0.08; }
  if (layout.filter((id) => id === 'yin_yang').length >= 2) { lines.push('Lưỡng Nghi Đối Ứng'); multiplier += 0.1; }
  if (unique.has('spirit')) { lines.push('Tinh Thần Diễn Pháp'); multiplier += 0.05; }
  if (unique.has('chaos')) { lines.push('Hỗn Độn Quy Nhất'); multiplier += 0.15; }
  if (exact) { lines.push('Trận Đồ Hoàn Chỉnh'); multiplier += 0.1; }
  return { exact, lines, multiplier };
}
...
export async function comprehendFormation(client, guildId, userId) {
  ...
  if (last && remainingMs > 0) return { ok: false, reason: 'cooldown', remainingMs, state };
  ...
  const unlockedNow = state.unlockedFormationIds.filter((id) => !before.has(id)).map((id) => FORMATION_DEFINITIONS[id]).filter(Boolean);
  await saveFormationState(client, guildId, userId, state);
  return { ok: true, insightGain, essenceGain, unlockedNow, state };
}
...
export async function upgradeActiveFormation(client, guildId, userId) {
  ...
  if (currentLevel >= 10) return { ok: false, reason: 'max_level', state, cost: 0 };
  if (state.formationEssence < cost) return { ok: false, reason: 'not_enough_essence', state, cost };
...
export function getFormationProgress(state) {
  const locked = Object.values(FORMATION_DEFINITIONS).filter((formation) => !state.unlockedFormationIds.includes(formation.id)).sort((a, b) => a.unlockInsight - b.unlockInsight);
