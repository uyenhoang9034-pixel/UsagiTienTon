...
export async function cultivate(client, guildId, userId) {
  ...
  const petCultivationPercent = amplifySafePetEffect(
    getWrapperCultivationPetBonus(profile),
    cavePetBonus,
  );
  const petStaminaRefundPercent = amplifySafePetEffect(
  ...
  try {
    ...
    const legacyPetPercent = amplifySafePetEffect(
...
export async function breakthrough(client, guildId, userId) {
  ...
  const petLossReduction = amplifySafePetEffect(
    getPetEffectValue(profile, 'breakthrough_loss_reduction'),
    cavePetBonus,
    { cap: 0.95 },
  );

  // Đây là flag tuyệt đối: Linh Thú Viên không được biến đổi hoặc cộng thêm.
