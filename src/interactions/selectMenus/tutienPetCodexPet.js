...
import {
  buildPetCodexDetailEmbed,
  buildPetCodexDetailRows,
  getPetCodexRarityForPet,
  normalizePetCodexRarity,
} from '../../services/cultivationPetCodexUI.js';

export default {
  name: 'tutien_pet_codex_pet',

  async execute(interaction, client, args = []) {
    const [ownerId, rarityArg] = args;
    ...
    const rarity =
      normalizePetCodexRarity(rarityArg) ||
      getPetCodexRarityForPet(petId);

    if (!petId || !rarity) {
      return interaction.reply({
        content: 'Không xác định được Linh Thú trong Đồ Giám.',
        flags: MessageFlags.Ephemeral,
      });
    }
    ...
    return interaction.update({
      embeds: [buildPetCodexDetailEmbed(profile, petId)],
      components: buildPetCodexDetailRows(ownerId, rarity),
    });
  },
};
